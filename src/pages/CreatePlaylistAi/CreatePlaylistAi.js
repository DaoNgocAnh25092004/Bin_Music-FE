...
import styles from './CreatePlaylistAi.module.scss';
...
function CreatePlaylistAi() {
    ...
    // Mutation create generate playlist
    const mutationGeneratePlaylist = useGenericMutation((data) =>
    ...
    const handleGeneratePlaylist = () => {
        ...
        mutationGeneratePlaylist.mutate(formData, {
            onSuccess: (data) => {
                setGeneratedPlaylist(data.songs);
                setNamePlaylist(data.title);
    ...
    const handleSavePlaylist = () => {
        const playlistData = {
            userId: id,
            name: namePlaylist,
            songs: generatedPlaylist.map((song) => song._id),
        };

        mutationCreatePlaylist.mutate(playlistData, {
            onSuccess: () => {
                toast.success('Playlist đã được lưu thành công!');
                navigate('/library'); // Navigate to the library page after saving
                // Reset the playlist and other states if needed
...
export default CreatePlaylistAi;
